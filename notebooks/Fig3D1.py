...
for N in [300,500,1000]:
    for trial in range(3):
        ...
        start = time.time()
        nmf = NMF(n_components = k + n_active_pathways + n_control_pathways)
        ...
        """
        start = time.time()
        Z, model_params = ZIFA.fitModel(np.log(X + 1), k + n_active_pathways + n_control_pathways)
        end = time.time()
        time_value.append(start - end)
        models.append("ZIFA")
        param_vals.append(N)
        """
        start = time.time()
        model = run_trials(scipy.sparse.coo_matrix(X), vcells=None, nfactors=k + n_active_pathways + n_control_pathways, 
...
sns.lineplot(
    data=df,
    x="N", y="time(s)", hue="model", style="model",
    markers=True, dashes=False
)
plt.savefig("simulations/time_N.svg")